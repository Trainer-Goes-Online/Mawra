...
// Alphabetical by name. Shared dial codes (US/CA share +1, RU/KZ share +7) are
// disambiguated by the SEED_ISO map below for the ISO lookup.
...
/** Emoji flag from an ISO-2 code, e.g. "IN" → 🇮🇳. */
...
// Preferred ISO for dial codes shared by more than one country.
const SEED_ISO: Record<string, string> = { "+1": "US", "+7": "RU" };

const DIAL_TO_ISO: Record<string, string> = { ...SEED_ISO };
for (const c of COUNTRIES) {
  if (!(c.dial in DIAL_TO_ISO)) DIAL_TO_ISO[c.dial] = c.iso;
}

/** Convert a dial code like "+91" to its ISO-2 country code ("IN"). "" if unknown. */
export function dialCodeToCountryIso(dialCode: string | undefined | null): string {
  if (!dialCode) return "";
  const key = dialCode.trim().startsWith("+")
    ? dialCode.trim()
    : `+${dialCode.trim()}`;
  return DIAL_TO_ISO[key] || "";
}
