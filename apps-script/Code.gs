/**
 * The Food Freedom Co. (Sreshtha) CRM — downstream Meta CAPI event firing engine.
 *
 * Bound to the Sreshtha CRM Google Sheet. On every USER edit to a trigger column
 ...
 * event_id + dedup flag. Failures are logged to the hidden `_Errors` tab so the
 * row remains retry-able.
 *
 * Funnel: ₹97 "1:1 Breakthrough Call" tripwire → 1:1 sales call → high-ticket
 * "Identity Evolution Programme". The tripwire `sales` event is fired separately
 * by the Next.js backend at payment-verify time (custom-only, Health & Wellness
 * posture — NO standard Purchase). This script handles ONLY the three downstream
 * events: CallBooked, CallDone, HighTicketPurchase. The two systems share the
 * same Meta pixel + token but never talk directly — the Sheet is the only link.
 *
 * Architecture: this script talks to Meta directly (no backend proxy). All
 * secrets live in Apps Script PropertiesService. A new client is onboarded by
 * copy-pasting this file into their Sheet's Apps Script editor and setting three
...
// Event names are bare/neutral (no condition words) — Health & Wellness safe.
const EVENTS = {
  CALL_BOOKED: {
...
function fireDownstreamEvent(sheet, row, cfg) {
  ...
  // custom_data — reporting metadata (not part of EMQ). PHI-free.
  const customData = {
    payment_id: leadId,
  };
  if (cfg.includeValue) {
  ...
  // Both should be origin-only (Health & Wellness posture).
  ...
  if (result.ok) {
    ...
    // Reading is handled by isTruthy() which accepts either.
...
function postToMetaCapi(payload) {
  ...
  // Optional test-event code for Events Manager → Test Events QA.
...
function setupTriggers() {
  ...
  console.log(msg);
  SpreadsheetApp.getActive().toast(msg, 'Sreshtha CRM', 5);
}
...
function replayPendingEvents() {
  ...
  SpreadsheetApp.getActive().toast('Replayed ' + replayed + ' pending event(s)', 'Sreshtha CRM', 5);
}
